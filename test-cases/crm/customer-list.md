...
- **Automated in:** [`tests/regression/crm/03-customer-list.spec.ts`](../../tests/regression/crm/03-customer-list.spec.ts) — built despite the duplication, per team decision not to skip duplicates. Same content/bodies as `create-customer.spec.ts`'s TC:3-11, pointed at this task's own ClickUp subtasks.
