 ...
 *
 * Locators confirmed against the real running screen (dumped via
 * ariaSnapshot() against a local `tilt up` stack). Two things worth
 * knowing:
 *
...
export class ContactListPage extends BasePage {
  ...
  async selectRelationshipTab(tab: 'All' | 'Linked' | 'Unlinked'): Promise<void> {
    const button = { All: this.allTab, Linked: this.linkedTab, Unlinked: this.unlinkedTab }[tab];
    await button.click();
  }

  async selectLayout(layout: 'No split' | 'Vertical split' | 'Horizontal split'): Promise<void> {
    const button = {
      'No split': this.noSplitButton,
      'Vertical split': this.verticalSplitButton,
      'Horizontal split': this.horizontalSplitButton,
    }[layout];
    await button.click();
  }
  ...
  async selectContactRow(contactName: string): Promise<void> {
    await this.dataRows.filter({ hasText: contactName }).first().click();
  }

  async selectLinkedCustomer(customerName: string): Promise<void> {
    await this.page.getByRole('link', { name: customerName }).click();
  }
}
